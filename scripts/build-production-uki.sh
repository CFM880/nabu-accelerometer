...
if [ "$#" -ne 3 ]; then
	echo "usage: $0 /path/to/linux /path/to/output /path/to/nabu-accelerometer-production.efi" >&2
	exit 2
fi
...
set -- build \
	...
	--cmdline "@$project_dir/config/production.cmdline" \
...
echo "built production SLPI accelerometer UKI: $destination"
echo "DTB: $dtb"
