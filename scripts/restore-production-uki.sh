...
[ "$(sha256sum "$backup_uki" | cut -d ' ' -f 1)" = "$original_default_sha256" ] || {
...
if [ "$current_sha256" = "$original_default_sha256" ]; then
...
sync "$temporary_uki"
[ "$(sha256sum "$temporary_uki" | cut -d ' ' -f 1)" = "$original_default_sha256" ] || exit 1
