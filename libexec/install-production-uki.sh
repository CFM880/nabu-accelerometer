...
#
# Legacy standalone sensor-UKI installer.  Production is now built and
# installed by nabu-main; the pinned SHA256 values below identify the last
# archived prebuilt sensor UKI and are not regenerated by this repository.
set -eu
...
strings "$source_uki" | grep -Fq \
	'root=PARTLABEL=linux rw fw_devlink=permissive modprobe.blacklist=venus_core module_blacklist=venus_core console=tty0 ignore_loglevel' || {
